...
const App = {
	...
	setupEventListeners() {
		...
		document.getElementById('urls-refresh-btn').addEventListener('click', () => {
			this.loadTopUrls();
		});
		document.getElementById('errors-refresh-btn').addEventListener('click', () => {
			this.loadErrors();
		});
	...
	async handleLogin() {
		...
		try {
			...
			errorDiv.textContent = error.message || 'Token inválido. Verifique e tente novamente.';
	...
	renderOverview(data) {
		...
		tbody.innerHTML = byStatusCode.map(item => {
			let badgeClass = 'badge-secondary';
			if (item.statusCode >= 200 && item.statusCode < 300) badgeClass = 'badge-success';
			else if (item.statusCode >= 400) badgeClass = 'badge-error';
			else if (item.statusCode >= 300 && item.statusCode < 400) badgeClass = 'badge-warning';
	...
	renderErrors(data) {
		...
		tbody.innerHTML = data.map(item => {
			...
			const errorMessage = item.errorMessage || '<em style="color: hsl(var(--muted-foreground));">Sem mensagem de erro</em>';
